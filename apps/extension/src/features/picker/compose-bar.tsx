      ...
      <div className="flex items-stretch gap-1.5">
        <div className="relative flex min-h-9 min-w-0 flex-1 items-center rounded-md border border-zinc-200 bg-zinc-50 px-2 dark:border-zinc-600 dark:bg-zinc-900">
          <input
            aria-live="polite"
            disabled
            className={`min-h-9 min-w-0 flex-1 text-md leading-none outline-none ${hasSelection ? 'text-zinc-900 dark:text-zinc-100' : 'text-zinc-400'
              }`}
            value={hasSelection ? selection : '…'}
          />
          {hasSelection && (
            <button
